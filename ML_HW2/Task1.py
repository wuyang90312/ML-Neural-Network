...
class SOFTMAX:
    ...
    def constru_SOFTMAX(self, images_val, labels_val, images_test, labels_test):
        ...
        logits = tf.add(tf.matmul(X/255, W), b)
        ...
        log_likelihood_valid = -tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(
            logits=tf.add(tf.matmul(images_val/255, W), b), labels=labels_val))

        train_predition = tf.nn.softmax(logits)
        valid_prediction = tf.nn.softmax(tf.add(tf.matmul(images_val/255, W), b))
        test_prediction = tf.nn.softmax(tf.add(tf.matmul(images_test/255, W), b))
        ...
        for epoch in range(training_epochs):
            for i in xrange(150):
                batch_xs = self.images[i * 100: (i + 1) * 100]
                batch_ys = self.labels[i * 100: (i + 1) * 100]
...
softmax = SOFTMAX(images_in[:15000,:], labels_in[:15000,:])
softmax.constru_SOFTMAX(images_in[15000:16000,:], labels_in[15000:16000,:], images_in[16000:18720,:], labels_in[16000:18720,:])
