...
class NN:
    ...
    def constru_NN(self, images_val, labels_val, images_test, labels_test):
        ...
        b1 = tf.Variable(tf.zeros([1000]), name='weight_b2h')
        w1 = tf.Variable(tf.truncated_normal([784, 1000], stddev=0.1), name='weight_i2h')
        b2 = tf.Variable(tf.zeros([10]), name='weight_b2o') 
        w2 = tf.Variable(tf.truncated_normal([1000, 10], stddev=0.1), name='weight_h2o') 
        ...
        # Set up 2 math operations: input -> hidden unit, hidden unit -> output and use the dropout with dropout rate 0.5 in hidden unit
        layer1_result = tf.nn.relu(tf.add(tf.matmul(X, w1), b1))

        layer1_result_dropout = tf.nn.dropout(layer1_result, dropout_rate)
        logits = tf.add(tf.matmul(layer1_result_dropout, w2), b2)
        ...
        logits_test = tf.add(tf.matmul(layer1_result_test, w2), b2)
        ...
        for epoch in range(training_epochs):
            for i in xrange(300):
                ...
                cost_np, _ = sess.run([cost,train_step], 
                    feed_dict={X: batch_xs, Y: batch_ys, dropout_rate: 0.5})
